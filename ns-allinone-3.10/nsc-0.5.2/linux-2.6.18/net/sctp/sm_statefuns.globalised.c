...
# 1153 "linux-2.6.18/net/sctp/sm_statefuns.c"
 ...
 for (pos = (&new_asoc->peer.transport_addr_list)->next; prefetch(pos->next), pos != (&new_asoc->peer.transport_addr_list); pos = pos->next) {
  new_addr = ({ const typeof( ((struct sctp_transport *)0)->transports ) *__mptr = (pos); (struct sctp_transport *)( (char *)__mptr - __builtin_offsetof(struct sctp_transport,transports) );});
  found = 0;
  for (pos2 = (&asoc->peer.transport_addr_list)->next; prefetch(pos2->next), pos2 != (&asoc->peer.transport_addr_list); pos2 = pos2->next) {
   addr = ({ const typeof( ((struct sctp_transport *)0)->transports ) *__mptr = (pos2); (struct sctp_transport *)( (char *)__mptr - __builtin_offsetof(struct sctp_transport,transports) );})
                  ;
