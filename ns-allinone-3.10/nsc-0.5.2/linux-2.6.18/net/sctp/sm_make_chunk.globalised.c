         ...
         sctp_addip_param_t *asconf_param)
{
 ...
 case SCTP_PARAM_ADD_IP:
  ...
  for (pos = (&bp->address_list)->next; prefetch(pos->next), pos != (&bp->address_list); pos = pos->next) {
   saddr = ({ const typeof( ((struct sctp_sockaddr_entry *)0)->list ) *__mptr = (pos); (struct sctp_sockaddr_entry *)( (char *)__mptr - __builtin_offsetof(struct sctp_sockaddr_entry,list) );});
       ...
       struct sctp_chunk *asconf_ack)
{
 ...
 if (!list_empty(&asoc->addip_chunk_list)) {
  ...
  asconf = ({ const typeof( ((struct sctp_chunk *)0)->list ) *__mptr = (entry); (struct sctp_chunk *)( (char *)__mptr - __builtin_offsetof(struct sctp_chunk,list) );});
