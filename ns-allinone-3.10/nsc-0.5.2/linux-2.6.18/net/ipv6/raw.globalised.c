...
static __inline__  __attribute__((always_inline)) int icmpv6_filter(struct sock *sk, struct sk_buff *skb)
{
 struct icmp6hdr *icmph;
...
static inline  __attribute__((always_inline)) int rawv6_rcv_skb(struct sock * sk, struct sk_buff * skb)
{
...
static struct sock *raw6_get_first(struct seq_file *seq)
{
 ...
 for (state->bucket = 0; state->bucket < 256; ++state->bucket)
  for (node = (&(*_GLOBAL_raw_v6_htable_50_A[get_stack_id()])[state->bucket])->first; node && ({ prefetch(node->next); 1;}) && ({ sk = ({ const typeof( ((typeof(*sk) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk) *)( (char *)__mptr - __builtin_offsetof(typeof(*sk),__sk_common.skc_node) );}); 1;}); node = node->next)
   if (sk->__sk_common.skc_family == 10)
    goto out;
 sk = ((void *)0);
out:
