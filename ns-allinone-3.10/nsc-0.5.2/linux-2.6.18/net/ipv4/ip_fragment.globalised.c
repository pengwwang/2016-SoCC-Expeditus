...
static void ipfrag_secret_rebuild(unsigned long dummy)
{
 ...
 for (i = 0; i < 64; i++) {
  ...
  for (p = (&(*_GLOBAL_ipq_hash_38_A[get_stack_id()])[i])->first; p && ({ n = p->next; 1; }) && ({ q = ({ const typeof( ((typeof(*q) *)0)->list ) *__mptr = (p); (typeof(*q) *)( (char *)__mptr - __builtin_offsetof(typeof(*q),list) );}); 1;}); p = n) {
...
static inline  __attribute__((always_inline)) struct ipq *ip_find(struct iphdr *iph, u32 user)
{
 ...
 for (n = (&(*_GLOBAL_ipq_hash_38_A[get_stack_id()])[hash])->first; n && ({ prefetch(n->next); 1;}) && ({ qp = ({ const typeof( ((typeof(*qp) *)0)->list ) *__mptr = (n); (typeof(*qp) *)( (char *)__mptr - __builtin_offsetof(typeof(*qp),list) );}); 1;}); n = n->next) {
  if(qp->id == id &&
     qp->saddr == saddr &&
     qp->daddr == daddr &&
