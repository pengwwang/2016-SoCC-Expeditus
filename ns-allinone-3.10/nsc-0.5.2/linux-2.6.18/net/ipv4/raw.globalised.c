        ...
        int dif)
{
 ...
 if (sk && ({ node = &(sk)->__sk_common.skc_node; 1; })) for (; node && ({ prefetch(node->next); 1;}) && ({ sk = ({ const typeof( ((typeof(*sk) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk) *)( (char *)__mptr - __builtin_offsetof(typeof(*sk),__sk_common.skc_node) );}); 1;}); node = node->next) {
...
static __inline__  __attribute__((always_inline)) int icmp_filter(struct sock *sk, struct sk_buff *skb)
{
 int type;
...
static struct sock *raw_get_first(struct seq_file *seq)
{
 ...
 for (state->bucket = 0; state->bucket < 256; ++state->bucket) {
  ...
  for (node = (&(*_GLOBAL_raw_v4_htable_52_A[get_stack_id()])[state->bucket])->first; node && ({ prefetch(node->next); 1;}) && ({ sk = ({ const typeof( ((typeof(*sk) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk) *)( (char *)__mptr - __builtin_offsetof(typeof(*sk),__sk_common.skc_node) );}); 1;}); node = node->next)
...
static __inline__  __attribute__((always_inline)) char *get_raw_sock(struct sock *sp, char *tmpbuf, int i)
{
