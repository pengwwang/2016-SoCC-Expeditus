...
static inline  __attribute__((always_inline)) int udp_lport_inuse(u16 num)
{
 ...
 for (node = (&(*_GLOBAL_udp_hash_38_A[get_stack_id()])[num & (128 - 1)])->first; node && ({ prefetch(node->next); 1;}) && ({ sk = ({ const typeof( ((typeof(*sk) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk) *)( (char *)__mptr - __builtin_offsetof(typeof(*sk),__sk_common.skc_node) );}); 1;}); node = node->next)
...
static int udp_v4_get_port(struct sock *sk, unsigned short snum)
{
 ...
 if (snum == 0) {
  ...
  for (i = 0; i < 128; i++, result++) {
   ...
   size = 0;
   for (node = (list)->first; node && ({ prefetch(node->next); 1;}) && ({ sk2 = ({ const typeof( ((typeof(*sk2) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk2) *)( (char *)__mptr - __builtin_offsetof(typeof(*sk2),__sk_common.skc_node) );}); 1;}); node = node->next)
...
gotit:
  ...
  for (node = (&(*_GLOBAL_udp_hash_52_A[get_stack_id()])[snum & (128 - 1)])->first; node && ({ prefetch(node->next); 1;}) && ({ sk2 = ({ const typeof( ((typeof(*sk2) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk2) *)( (char *)__mptr - __builtin_offsetof(typeof(*sk2),__sk_common.skc_node) );}); 1;}); node = node->next)
                                                {
       ...
       u32 daddr, u16 dport, int dif)
{
 ...
 for (node = (&(*_GLOBAL_udp_hash_52_A[get_stack_id()])[hnum & (128 - 1)])->first; node && ({ prefetch(node->next); 1;}) && ({ sk = ({ const typeof( ((typeof(*sk) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk) *)( (char *)__mptr - __builtin_offsetof(typeof(*sk),__sk_common.skc_node) );}); 1;}); node = node->next) {
  struct inet_sock *inet = inet_sk(sk);
...
static __inline__  __attribute__((always_inline)) struct sock *udp_v4_lookup(u32 saddr, u16 sport,
          u32 daddr, u16 dport, int dif)
{
 struct sock *sk;
...
static inline  __attribute__((always_inline)) struct sock *udp_v4_mcast_next(struct sock *sk,
          ...
          int dif)
{
 ...
 if (s && ({ node = &(s)->__sk_common.skc_node; 1; })) for (; node && ({ prefetch(node->next); 1;}) && ({ s = ({ const typeof( ((typeof(*s) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*s) *)( (char *)__mptr - __builtin_offsetof(typeof(*s),__sk_common.skc_node) );}); 1;}); node = node->next) {
...
static int udp_push_pending_frames(struct sock *sk, struct udp_sock *up)
{
 ...
 if (skb_queue_len(&sk->sk_write_queue) == 1) {
  ...
  if (skb->ip_summed == 1) {
   skb->csum = __builtin_offsetof(struct udphdr,check);
...
static __inline__  __attribute__((always_inline)) int __udp_checksum_complete(struct sk_buff *skb)
{
 return __skb_checksum_complete(skb);
}

static __inline__  __attribute__((always_inline)) int udp_checksum_complete(struct sk_buff *skb)
{
 return skb->ip_summed != 2 &&
...
static struct sock *udp_get_first(struct seq_file *seq)
{
 ...
 for (state->bucket = 0; state->bucket < 128; ++state->bucket) {
  struct hlist_node *node;
  for (node = (&(*_GLOBAL_udp_hash_52_A[get_stack_id()])[state->bucket])->first; node && ({ prefetch(node->next); 1;}) && ({ sk = ({ const typeof( ((typeof(*sk) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk) *)( (char *)__mptr - __builtin_offsetof(typeof(*sk),__sk_common.skc_node) );}); 1;}); node = node->next) {
