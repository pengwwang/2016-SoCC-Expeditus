...
static inline  __attribute__((always_inline)) void __inode_dir_notify(struct inode *inode, unsigned long event)
{
}

static inline  __attribute__((always_inline)) void dnotify_flush(struct file *filp, fl_owner_t id)
{
}

static inline  __attribute__((always_inline)) int fcntl_dirnotify(int fd, struct file *filp, unsigned long arg)
{
 return -22;
}

static inline  __attribute__((always_inline)) void dnotify_parent(struct dentry *dentry, unsigned long event)
{
}

static inline  __attribute__((always_inline)) void inode_dir_notify(struct inode *inode, unsigned long event)
{
}
                          ...
                          struct notifier_block *n)
{
        while ((*nl) != ((void *)0)) {
         if ((*nl) == n) {
               ({ __asm__ __volatile__("": : :"memory"); (*nl) = (n->next); });
               return 0;
         }
         nl = &((*nl)->next);
        }
