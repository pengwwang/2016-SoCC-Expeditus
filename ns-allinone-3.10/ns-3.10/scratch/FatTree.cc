...
#include "ns3/fat-tree-cp-helper.h"
	...
	main(int argc, char *argv[])
{
	...
	double linkBw = 1e10;		// Link bandwidth in bps	
       int torosp = 2;			// ToR tier oversubscription ratio
       	double coreosp = 1;       // Core tier oversubscription ratio
...
//double flowletGap = 2; // default 200us for flowlet
	...
	cmd.AddValue ("k", "Number of pods", k);	
      ...
      cmd.AddValue ("coreosp", "Oversubscription ratio at Core tier", coreosp);	
	...
	cmd.Parse (argc, argv);
        ...
        Config::SetDefault ("ns3::HashRouting::CoreOsp", DoubleValue(coreosp));
...
//	 Config::SetDefault ("ns3::TcpSocket::RcvBufSize", UintegerValue(13107200));
	...
	int total_host = torosp*k*k*k/4;	// Number of hosts in the entire network
	
	// Create the Fat-tree topology
	NodeContainer hosts;
	Ptr<FatTreeCpHelper> net = CreateObject<FatTreeCpHelper>(n);
...
//      char *homedir = pw->pw_dir;
	if (cdftype == 1)
		sprintf(filename, "/proj/expeditus/SOCC-Results-Fattree/DCTCP/Run-%d-%d-%.2f-%.4f-%.1f-%d-%.1f-%d.xml", run, scheme, tSim, flowletGap, load, k, coreosp, torosp);

	else if (cdftype == 2)	
		sprintf(filename, "/proj/expeditus/SOCC-Results-Fattree/VL2/Run-%d-%d-%.2f-%.4f-%.1f-%d-%.1f-%d.xml", run,  scheme, tSim, flowletGap, load, k, coreosp, torosp);
	else {
...
//		interarrival = static_cast<double>(8.0*1134/total_host) / static_cast<double>((linkBw/1e3)*load); 
	// Hong Zhang
		interarrival = static_cast<double>(8.0*1134/(total_host/(coreosp*torosp))) / static_cast<double>((linkBw/1e3)*load); 
	}
        if (cdftype == 2) {
                // VL2 mean flow size is 2126KB
//                interarrival = static_cast<double>(8.0*2126/total_host) / static_cast<double>((linkBw/1e3)*load);
// Hong Zhang
                interarrival = static_cast<double>(8.0*2126/(total_host/(coreosp*torosp))) / static_cast<double>((linkBw/1e3)*load);
        } 
	...
	if (cdftype == 1) {		// DCTCP traffic trace
		...
		fsize.CDF (66667.0, 0.99); // Hong Zhang 
