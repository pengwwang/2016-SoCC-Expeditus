...
OnOffApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::OnOffApplication")
    ...
    .AddAttribute ("PrioThresh", 
                   " Prioritize small flows whose size if below PrioThresh "
                   " The value zero means that priority scheme is not applied.",
                   UintegerValue (0),
                   MakeUintegerAccessor (&OnOffApplication::m_prioThresh),
                   MakeUintegerChecker<uint32_t> ())
