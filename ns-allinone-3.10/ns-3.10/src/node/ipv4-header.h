...
typedef struct __ipv4Options {
//   uint32_t PathID;  // the PathID (core switch address for inter-pod traffic, aggregation switcch for intra-pod traffic)
  uint8_t lbTag;
  uint8_t ce;
  uint8_t fb_lbTag;
  uint8_t fb_metric; 
  uint32_t dqueue;  // buffer occupancy of output port leading to host (used by Expeditus and Expeditus-Pathlet)
  uint32_t uqueue;  // buffer occupancy of output port leading to core/aggregation switch (used by Expeditus and Expeditus-Flowlet)
} Ipv4Options;
...
public:
 ...
 /**
   * \param PathID the aggregation switch route this packet pass through
   */
  void SetPathID (uint32_t PathID);

  void SetLBTag (uint8_t path);

  void SetCE (uint8_t queue);
 
  void SetFB_LBTag (uint8_t path);

  void SetFB_Metric (uint8_t queue);

/**
   *\param queue buffer occupancy of output port leading to host collected in the IP header 
   */
  void SetDqueue (uint32_t queue);
  /**
   *\param queue buffer occupancy of output port leading to core/aggregation switch collected in the IP header
   */
  void SetUqueue (uint32_t queue);
  ...
  /**
   * \returns the PathID aggregation switch address) of this packet
   */
   uint32_t GetPathID (void);

   uint8_t GetLBTag (void);

   uint8_t GetCE (void);

   uint8_t GetFB_LBTag (void);

   uint8_t GetFB_Metric (void);

 /**
   *\returns buffer occupancy of output port leading to host collected in the IP header 
   */
   uint32_t GetDqueue (void);
  /**
   *\returns buffer occupancy of output port leading to switch collected in the IP header collected in the IP header 
   */
   uint32_t GetUqueue (void);
